...
var CanvizEntity = Class.create({
	...
	draw: function(ctx, ctxScale, redrawCanvasOnly) {
	  var i, tokens, fillColor, strokeColor, bbDiv;
		if (!redrawCanvasOnly) {
			this.initBB();
			bbDiv = new Element('div');
			if (this.defaultAttrHashName != 'edgeAttrs') {
			  bbDiv.setAttribute('id', 'elt_'+this.name);
			  bbDiv.setAttribute('class', 'elt_bbox');
			}
...
//									debug(this.name + ', href ' + href + ', target ' + target + ', tooltip ' + tooltip);
								...
								text.update(str);
								text.setAttribute('abs_left', (l - (1 + textAlign) * textWidth) + '');
								text.setAttribute('abs_top', t + '');
								text.setStyle({
...
//							debug('set font ' + fontSize + 'pt ' + fontFamily);
				...
				if (!redrawCanvasOnly) {
					var left = Math.round(ctxScale * this.bbRect.l + this.canviz.padding);
					var top = Math.round(ctxScale * this.bbRect.t + this.canviz.padding);
					var width = Math.round(ctxScale * this.bbRect.getWidth());
					var height = Math.round(ctxScale * this.bbRect.getHeight());
					bbDiv.setStyle({
						position: 'absolute',
						left:   left + 'px',
						top:    top + 'px',
					...
					// Position children (text labels) correctly
					bbDiv.childElements().each(function(elt) {
						var myLeft = parseInt(elt.getAttribute('abs_left'));
						var myTop = parseInt(elt.getAttribute('abs_top'));
												   console.log(myLeft, left);
						elt.setStyle({left: (myLeft-left) + 'px',
									  top: (myTop-top) + 'px'});
					});
				}
				ctx.restore();
...
var Canviz = Class.create({
	...
	nodeObserve: function (evt, func) {
		this.nodeObservers.set(evt, func);
		this.updateEventHandlers();
	},
	updateEventHandlers: function() {
		$(this.elements).childElements().each(function(elt) {
			Event.stopObserving(elt);
			if (elt.getAttribute('class') == 'elt_bbox'){
				this.nodeObservers.each(function(pair) {
					elt.observe(pair.key, pair.value);
				});
			}
		}.bind(this));
	},
